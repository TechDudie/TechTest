...
local MACHINE_TICK_FUNC = function( machine, dt )
    if machine.state == GRAVELYZE_STATE_IDLE then
        ...
        if input_inv:contains_item(input_list, 'techtest:pyrite') then
            -- if input has gravel, remove one piece
            input_inv:remove_item(input_list, 'techtest:pyrite')
    ...
    if machine.state == GRAVELYZE_STATE_PROCESSING then
        ...
        if terumet.machine.expend_heat(machine, 2, 'Gravelyzing') then
            ...
            if machine.state_time <= 0 then
                ...
                if output_inv:room_for_item(output_list, 'technic:sulfur_lump') the
                    -- place a flint in output inventory
                    if math.random(0,100) <= 15 then
                        output_inv:add_item(output_list, 'technic:sulfur_lump')
                        -- set machine's state to idle
                        machine.state = GRAVELYZE_STATE_IDLE
                        machine.status_text = 'Extracting finished!'
                else
                    -- if no room, let the player know
                    machine.status_text = 'Extracting finished but no room for output.'
                    -- keep state time just below zero to trigger attempt to output next tick
                    machine.state_time = -0.01
                end
                if output_inv:room_for_item(output_list, 'technic:lump') the
                    -- place a flint in output inventory
                    if math.random(0,100) <= 15 then
                        output_inv:add_item(output_list, 'technic:sulfur_lump')
                        -- set machine's state to idle
                    ...
                    -- if no room, let the player know
                    machine.status_text = 'Extracting finished but no room for output.'
                    -- keep state time just below zero to trigger attempt to output next tic
            else
...
local gravelyzer_data = {
    ...
    machinefs_theme = 'background[0,0;11,9;extractor_bg.png;true]',
    ...
    machinefs_func = function( machine )
        if machine.state == GRAVELYZE_STATE_PROCESSING then
            ...
            return terumet.machine.fs_proc(3,1.5, 'gen', ITEM)
        else
            return ""
        end
    end,
...
terumet.register_heat_machine( 'techtest:extractor', gravelyzer_data )

-- register a recipe for our machine
minetest.register_craft{ output='techtest:extractor', recipe={
    {'','default:flint',''},
